...
const tempRoot = path.join(os.tmpdir(), `xapay-admin-build-${process.pid}`)
const tempProjectRoot = path.join(tempRoot, 'workspace')
const staticOutputRoot = path.join(workspaceRoot, 'backend', 'public', 'admin')
...
async function syncProjectToTemp() {
  ...
  await cp(projectRoot, tempProjectRoot, {
    ...
    filter(source) {
      ...
      if (normalized === path.join(projectRoot, 'dist')) return false
      if (normalized === path.join(projectRoot, '.tmp')) return false
      return true
    },
  })
}
