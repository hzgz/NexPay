...
export function createAuthState() {
  ...
  const captcha = reactive<Record<string, any>>({ captcha_key: '', captcha_code: '', captcha_image: '', captcha_hint: '' })
...
export function createAuthFlags(authConfig: AuthConfigState, options: AuthFlagOptions = {}) {
  ...
  return { needCaptcha, needGeetest }
}
...
export function applyCaptcha(target: CaptchaState, source?: Record<string, any>) {
  ...
  Object.assign(target, {
    ...source,
    captcha_code: '',
    captcha_image: source.captcha_image || '',
    captcha_hint: source.captcha_hint || '请输入图片中的验证码',
  })
  target.captcha_code = ''
}
