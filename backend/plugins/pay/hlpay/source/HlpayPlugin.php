...
namespace plugins\payment\hlpay;

use app\common\PaymentContext;
use app\common\BasePayment;
use Exception;

class HlpayPlugin extends BasePayment
{
    ...
    public function alipay(PaymentContext $ctx): array
    {
        ...
        try {
            $result = $this->addOrder($ctx, 'ALIPAY', $pay_sub_type);
    ...
    public function alipayjs(PaymentContext $ctx): array
    {
        ...
        try {
            $result = $this->addOrder($ctx, 'ALIPAY', 'JSAPI', null, $user_id);
    ...
    public function wxpay(PaymentContext $ctx): array
    {
        ...
        if (in_array('2', $this->channel['apptype']) && !in_array('1', $this->channel['apptype'])) {
            ...
            try {
                $result = $this->addOrder($ctx, 'WECHAT', 'NATIVE');
                $code_url = $result['payInfo'];
            } catch (Exception $ex) {
    ...
    public function wxjspay(PaymentContext $ctx): array
    {
        ...
        try {
            $result = $this->addOrder($ctx, 'WECHAT', $ctx->order['is_applet'] == 1 ? 'MINI_APP' : 'JSAPI', $wxinfo['appid'], $openid);
        } catch (Exception $ex) {
            return ['type' => 'error', 'msg' => '微信支付下单失败！' . $ex->getMessage()];
        }
        if ($ctx->method == 'jsapi') {
    ...
    public function wxminipay(PaymentContext $ctx): array
    {
        ...
        try {
            $result = $this->addOrder($ctx, 'WECHAT', 'MINI_APP', $wxinfo['appid'], $openid);
        } catch (Exception $ex) {
    ...
    {
        try {
            $result = $this->addOrder($ctx, 'UNION_PAY', 'NATIVE');
        } catch (Exception $ex) {
    ...
    public function query(array $order): array
    {
        ...
        $result = $client->execute('/openapi/pay/query', $param);
        return [
