...
namespace plugins\payment\wxpay;

use app\common\PaymentContext;
use app\common\BasePayment;

class WxpayPlugin extends BasePayment
{
    ...
    private function getConfig(): array
    {
        return [
            ...
            //付款到银行卡的RSA公钥
            'publickey_path' => getCertFilePath('wxpaypubkey_'.$this->channel['appmchid'].'.pem', true),
        ];
    }
    ...
    //转账
    public function transfer($bizParam)
    {
        $money = strval($bizParam['money'] * 100);
        try {
            $client = new \WeChatPay\TransferService($this->wechatpayConfig);
            if ($bizParam['type'] == 'bank') {
                $bank_code = $this->getBankCode($bizParam['payee_account']);
    ...
    //转账查询
    public function transfer_query($bizParam)
    {
        try {
            $client = new \WeChatPay\TransferService($this->wechatpayConfig);
            if ($bizParam['type'] == 'bank') {
    ...
    private function getBankCode($bankcard)
    {
        $bank_info = getBankCardInfo($bankcard);
        $bank_code_arr = json_decode(file_get_contents($this->payRoot . 'inc/bankcode.json'), true);
