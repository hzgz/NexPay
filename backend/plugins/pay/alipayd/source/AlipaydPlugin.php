...
namespace plugins\payment\alipayd;

use app\common\PaymentContext;
use app\common\BasePayment;

class AlipaydPlugin extends BasePayment
{
    ...
    public function submitwap(PaymentContext $ctx, bool $useReturnUrl = false): array
    {
        ...
        try {
            $aop = new \Alipay\AlipayTradeService($config);
            if ($this->isCombinePay($ctx->order['realmoney'])) {
                $bizContent['product_code'] = 'QUICK_WAP_WAY';
                $bizContent = $this->combineOrderParams($bizContent, $sub_orders);
    ...
    public function apppay(PaymentContext $ctx): array
    {
        ...
        try {
            ...
            if ($this->isCombinePay($ctx->order['realmoney'])) {
                ...
                $this->updateOrderCombine($ctx->order['trade_no'], $sub_orders);
            } else {
                $aop->directPayParams($bizContent);
    ...
    {
        if ($ctx->order['combine']) {
            return $this->combine_notify($ctx);
        }
        ...
        if ($verify_result) {
            ...
            if ($trade_status === 'TRADE_FINISHED') {
                ...
                if ($out_trade_no === $order['trade_no'] && round($total_amount, 2) == round((float) $order['realmoney'], 2)) {
                    if (config_get('direct_settle_time') > 0 && $order['profits'] == 0) {
                        $order['settle'] = 1;
                    } else {
                        usleep(300000);
                        try {
                            $freeze = config_get('direct_settle_time') > 0 && $order['profits'] > 0;
                            ...
                            if (strpos($errmsg, 'ALREADY_CONFIRM_SETTLE') !== false) {
                                ...
                                $order['settle'] = 3;
                                \app\lib\Payment::alipayd_settle_fail($this->channel, $order, $errmsg);
                            }
                        }
                    }
                    ($this->markTrustedCallback($ctx, 'notify', 'alipayd-rsa-check'))(function () use ($order, $trade_no, $buyer_id, $end_time) {
    ...
    public function return(PaymentContext $ctx): array
    {
        ...
        if ($verify_result) {
            ...
            if ($out_trade_no === $order['trade_no'] && round($total_amount, 2) == round((float) $order['realmoney'], 2)) {
                return ($this->markTrustedCallback($ctx, 'return', 'alipayd-rsa-check'))(function () use ($order, $trade_no) {
    ...
    public function preauthnotify(PaymentContext $ctx): array
    {
        ...
        if ($verify_result) {
            ...
            if ($out_trade_no === $ctx->order['trade_no']) {
                ...
                ($this->markTrustedCallback($ctx, 'notify', 'alipayd-preauth-check'))(function () use ($ctx, $trade_no, $buyer_id) {
    ...
    public function appgw(PaymentContext $ctx): array
    {
        ...
        if ($verify_result) {
            ...
            } elseif ($msg_method === 'ant.merchant.expand.indirect.zft.passed') {
                $bizContent = json_decode($biz_content, true);
            ...
            } elseif ($msg_method === 'ant.merchant.expand.indirect.zft.rejected') {
                $bizContent = json_decode($biz_content, true);
                if ($bizContent && isset($bizContent['order_id'])) {
     ...
     * 判断是否支持合单支付
     */
    private function isCombinePay($money): bool
    {
        if (!config_get('alicombine_open') || !config_get('wxcombine_minmoney')) return false;
        if ($money < config_get('wxcombine_minmoney')) return false;
        return true;
    }
     ...
     * 合单支付参数
     */
    private function combineOrderParams(array $bizContent, ?array &$sub_orders): array
    {
        $sub_mchid = $this->channel['appmchid'];
        $sub_mchids = null;
        if (strpos($this->channel['appmchid'], ',')) {
            $sub_mchids = explode(',', $this->channel['appmchid']);
            shuffle($sub_mchids);
        }
        ...
        if ($subnum < 6 && config_get('wxcombine_submoney')) {
            while ($submoney > intval(config_get('wxcombine_submoney') * 100)) {
                $subnum++;
                $submoney = intval($money / $subnum);
                if ($subnum == 6) break;
            }
     ...
     * 合单支付异步回调
     */
    private function combine_notify(PaymentContext $ctx): array
    {
        $aop = new \Alipay\AlipayService($this->alipayConfig);
        $verify_result = $aop->check(request()->post());

        if ($verify_result) {
            $out_trade_no = request()->post('out_merge_no');
            $buyer_id = request()->post('buyer_id');
            $order_detail_results = json_decode(request()->post('order_detail_results', '[]'), true);
            $order = $ctx->order;

            if (request()->post('merge_pay_status') === 'FINISHED' && $out_trade_no === $ctx->order['trade_no']) {
                $sub_orders = [];
                ...
                if (config_get('direct_settle_time') > 0 && $order['profits'] == 0) {
                    ...
                    $freeze = config_get('direct_settle_time') > 0 && $order['profits'] > 0;
                    $failnum = 0;
                    $tradeAop = new \Alipay\AlipayTradeService($this->alipayConfig);
                    foreach ($order_detail_results as $detail) {
                        $settle = 0;
                        try {
                            $tradeAop->settle_confirm($detail['trade_no'], $detail['total_amount'], $freeze);
                            $settle = 1;
                        } catch (\Throwable $e) {
                            $errmsg = $e->getMessage();
                            if (strpos($errmsg, 'ALREADY_CONFIRM_SETTLE') !== false) {
                                ...
                                $failnum++;
                                $tradeAop->writeLog('settle_confirm:' . $errmsg);
                            }
                        }
                        if ($settle == 1) $this->updateSubOrderSettle($detail['out_trade_no'], 1);
                    }
                ...
                ($this->markTrustedCallback($ctx, 'notify', 'alipayd-combine-check'))(function () use ($order, $out_trade_no, $buyer_id) {
                    $this->processNotify($order, $out_trade_no, $buyer_id);
                });
            }
            return ['type' => 'html', 'data' => 'success'];
        } else {
     ...
     * 合单支付同步回调
     */
    private function combine_return(): array
    {
