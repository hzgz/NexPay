...
class PayPalClient
{
    ...
    public function __construct(string $client_id, string $client_secret, int $mode)
    {
    ...
    //支付订单
    public function captureOrder(string $id): array
    {
    ...
    //查询支付
    public function paymentDetail(string $capture_id): array
    {
    ...
    //退款
    public function refundPayment(string $capture_id, array $params): array
    {
    ...
    //退款查询
    public function refundDetail(string $refund_id): array
    {
        $path = '/v2/payments/refunds/' . $refund_id;
        return $this->curl($this->gateway_url . $path);
    }

    private function curl(string $url, mixed $data = null, bool $auth = false): array
    {
        $header = [];
