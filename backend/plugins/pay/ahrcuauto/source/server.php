...
if (!$channel || ($channel['plugin'] ?? '') !== 'ahrcuauto') {
...
while (true) {
    ...
    if (!$currentChannel || ($currentChannel['plugin'] ?? '') !== 'ahrcuauto') {
        echo '通道不存在或插件不匹配' . PHP_EOL;
        goto WAIT;
    }

    $plugin = new \plugins\payment\ahrcuauto\AhrcuautoPlugin($currentChannel);
    $processed = 0;
    try {
