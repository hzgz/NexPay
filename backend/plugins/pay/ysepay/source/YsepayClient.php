...
class YsepayClient
{
    private string $gatewayUrl = 'https://ysgate.ysepay.com';
    ...
    public function __construct(string $partnerId, string $privateKeyPwd, string $certFilePath)
    {
        $this->partnerId = $partnerId;
        $this->privateKeyPwd = $privateKeyPwd;
        $this->businessgateCertPath = PLUGIN_ROOT . 'payment/ysepay/cert/businessgate.cer';
    ...
    //页面跳转支付
    public function pageExecute(string $method, array $bizParams): string
    {
    ...
    //发起请求
    public function request(string $path, string $method, string $version, array $bizContent, ?\CURLFile $file = null): mixed
    {
        $url = $this->gatewayUrl . $path;
        $key = random(16);
        $bizContentJson = json_encode($bizContent, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
        $bizContentEncrypted = $this->aesEncrypt($bizContentJson, $key);
        $params = [
            'method' => $method,
            'timeStamp' => date("Y-m-d H:i:s"),
            'charset' => 'utf-8',
    ...
    private function aesDecrypt(string $data, string $key): string|false
    {
    ...
    //异步通知回调验签
    public function verify2(array $params): bool
    {
        if (!$params || !isset($params['sign'])) {
            return false;
        }
    ...
    //验证返回内容签名
    private function verifyResponse(YsepayResponse $response): void
    {
    ...
    //私钥加签
    private function rsa256PrivateSign(string $data): string
    {
        $prikey = $this->getPrivateKey();
    ...
    //公钥验签
    public function rsa256PubilcVerify(string $data, string $sign): bool
    {
        $pubkey = $this->getPublicKey();
