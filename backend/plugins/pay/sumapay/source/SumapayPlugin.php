...
namespace plugins\payment\sumapay;

use app\common\PaymentContext;
use Exception;

class SumapayPlugin extends \app\common\BasePayment
{
    ...
    private function qrcode(PaymentContext $ctx): string
    {
        ...
        $signKeys = ['requestType', 'requestId', 'merchantCode', 'totalBizType', 'totalPrice', 'goodsDesc', 'rePayTimeOut', 'noticeUrl', 'passThrough'];
    ...
    private function h5apppay(PaymentContext $ctx, string $envflag = '0'): array
    {
        ...
        $signKeys = ['requestType', 'requestId', 'merchantCode', 'totalBizType', 'totalPrice', 'goodsDesc', 'envFlag', 'rePayTimeOut', 'noticeUrl', 'backUrl', 'passThrough'];
        $signatrue = $client->getSign($param, $signKeys);
    ...
    public function refundnotify(PaymentContext $ctx): array
    {
        ...
        if ($client->verify(request()->post(), $signKeys)) {
            ...
            ($this->markTrustedCallback($ctx, 'refundnotify', 'sumapay-signature'))(function () use ($status) {
                $this->processRefund(
