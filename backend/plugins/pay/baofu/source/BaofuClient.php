 ...
 * https://docs.baofu.com/docs/interface_document/baofujuhe
 */
class BaofuClient
{
    public string $gateway_url = 'https://juhe.baofoo.com/api';
    ...
    public function __construct(string $merId, string $terId, string $privateKeyPwd, string $publicCertPath, string $privateCertPath)
    {
        $this->merId = $merId;
        $this->terId = $terId;
    ...
    public function execute(string $method, array $bizContent): array
    {
        ...
        if (isset($result['returnCode']) && $result['returnCode'] == 'SUCCESS') {
            ...
            throw new \Exception($result['returnMsg'] ?? '返回数据解析失败');
        }
    }

    //代付接口
    public function transfer(string $method, array $bizContent): array
    {
        ...
        if (isset($arr['trans_content']['trans_head']['return_code'])) {
            if ($arr['trans_content']['trans_head']['return_code'] == '0000') {
                ...
                throw new \Exception($arr['trans_content']['trans_head']['return_msg'] ?? '返回数据解析失败');
            }
        } else {
    ...
    //统一入口
    public function submit(string $service_tp, array $params): array
    {
    ...
    //文件上传接口
    public function upload(array $params, string $file_name, string $file_path): array
    {
    ...
    //商户私钥加密
    private function rsaPrivateEncrypt(string $data): string
    {
        $pkcs12 = file_get_contents($this->privateCertPath);
        if (!openssl_pkcs12_read($pkcs12, $keyarr, $this->privateKeyPwd)) {
            throw new \Exception('商户私钥证书解析失败');
    ...
    //平台公钥解密
    public function rsaPublicDecrypt(string $data): string|false
    {
        $keyFile = file_get_contents($this->publicCertPath);
